function [CalibrationData] = CalibrationGUI(image, CalibrationData)

LINES = [];
LINES_0 = [];

if (nargin < 2 || isempty(CalibrationData))
...
BeamWavelength = CalibrationData.Lambda;
...
if (nargin < 1 || isempty(image))
    ...
    image=read2D(strcat(PATHNAME,FILENAME));
end

PilatusBlankRows = [[1:17] + 195, [1:17] + 17 + 195 * 2];

%displayedImage = double(image);
displayedImage =log10(1+abs(double(image)));
processedImage = double(image);

[M,N] = size(image);

Width_axes=12*N/M; Imax=max(displayedImage(:)); Imin=min(displayedImage(:));
...
fig_h = figure('Units','centimeters','Position',[1 2 9+Width_axes 15]);
...
popup_h = uicontrol('parent',cframe_h,'Style', 'popup','String', 'HDPE|AgBh|LAB6|Other','Backgroundcolor',[1 1 1],'units','centimeters','Position', [0.5 8.5 4 1],...
...
bgroup_h = uibuttongroup('Parent',cframe_h,'visible','on','units','centimeters','Position',[1 0.0 1 6.5],'bordertype','none','SelectionChangeFcn',@PeakSelectionCallback);
...
1;
    ...
    function Auto_callback(hObject,eventdata)
        ...
        gaussianAmplitudeThreshold = PercentileValue(image, 0.99);
        ...
        [X, Y, Values] = find(image>selectedThreshold);
        beamcenterX = mean(X);
        beamcenterY = mean(Y);
        ...
        for a = angles
            ...
            maxLength = sqrt(2) * max(size(image));
            ...
            ind = find(x > size(image, 1), 1);
            if (~isempty(ind)); x = x(1:ind-1); y = y(1:ind-1); end;
            
            ind = find(y > size(image, 2), 1);
            if (~isempty(ind)); x = x(1:ind-1); y = y(1:ind-1); end;
            
            r = sqrt((x - beamcenterX) .^ 2 + (y - beamcenterY) .^ 2);
            profile = interp2(1:size(image, 1), 1:size(image, 2), image', x, y, 'linear');
    ...
    function Done_callback(hObject,eventdata)
        ...
        CS=get(popup_h,'Value'); if CS==1, Sample='HDPE'; elseif CS==2, Sample='AgBh'; else Sample='Other'; end
    ...
    function PeakSelectionCallback(source,eventdata)
    ...
    function Initializes_calibration_values(hObject,eventdata)
        ...
        %%% HDPE
        Calib(1).Spacings=[4.166 3.78 3.014 2.49]; %Angstroms
        Calib(1).Angles_deg=2*asin(0.5*lambda./Calib(1).Spacings)*180/pi; %degrees
        
        %%% AgBH
        Calib(2).Spacings=58.38./(1:8); %Angstroms
        Calib(2).Angles_deg=2*asin(0.5*lambda./Calib(2).Spacings)*180/pi; %degrees
        ...
        Calib(3).Spacings = LAB6_Spacing(1:8); %Angstroms
        %Calib(3).Angles_deg = 2*asin(0.5*lambda./LAB6_Spacing)*180/pi; %degrees
        
        % Values given with the calibrant powder
        Calib(3).Angles_deg = [21.352, 30.35, 37.412, 43.481, 48.935, 53.962, 63.194, 67.521];
    ...
    function FirstImageDraw()
        axes(ax_h);
        hold off
        calibImage_h = imagesc(displayedImage); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 end
    ...
    function [xy]=get_point_zoom(xy)
        ...
        while not(Done)
            ...
            Id=image(yl(1):yl(2),xl(1):xl(2)); Id=medfilt2(Id,[3 3],'symmetric');
