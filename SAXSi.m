...
% Last Modified by GUIDE v2.5 03-Aug-2017 11:32:52
...
function Status(str, hObject, handles)
% function Status(str, hObject, handles)
...
if (~exist('hObject', 'var') || isempty(hObject))
    if (~exist('handles', 'var') || isempty(handles))
        ...
        try
            if (isempty(handles))
                hObject = evalin('caller', 'hObject');
                handles = guidata(hObject);
            end
        catch
            error('Could not automatically infer calling graphical object');
        end
    end
else
    if (~exist('handles', 'var') || isempty(handles))
...
try
    Status('SAXSi initializing... (<< 1 sec)');
    ...
    handles.State.IsWithinLoadSettings = 0;
    
    set(handles.DisplayedFilesList, 'Value', []);
    ...
    try
        LoadDisplaySettings(hObject);
    catch err
        timeStr = datestr(datetime('now','TimeZone','local'), 'yyyyMMddHHmmss');
        newFilename = sprintf('settings.display.%s.mat', timeStr);
        movefile('settings.display.mat', newFilename);
        h = errordlg(sprintf('There was some problem loading the display settings.\nSAXSi has renamed "settings.display.mat" to "%s".', newFilename));
        waitfor(h);
    end
    
    Status('Loaded display settings');
    
    % Try to load settings
    try
        LoadDefaultIntegrationSettings(hObject);
    catch err
        timeStr = datestr(datetime('now','TimeZone','local'), 'yyyyMMddHHmmss');
        newFilename = sprintf('settings.integration.%s.sxs', timeStr);
        movefile('settings.integration.sxs', newFilename);
        h = errordlg(sprintf('There was some problem loading the default settings.\nSAXSi has renamed "settings.integration.mat" to "%s".', newFilename));
        waitfor(h);
    end
    
    Status('Loaded integration settings');
    
    handles = guidata(hObject);
    handles.State.IsWithinLoadSettings = 1;
    ...
    plotTypeBox_Callback(hObject, [], handles);
    ...
    if (isdeployed() || ~(strcmp(lower(getenv('COMPUTERNAME')), 'jeremyah') || strcmp(lower(getenv('COMPUTERNAME')), 'japtop') || strcmp(lower(getenv('COMPUTERNAME')), 'avinery')))
...
%if (isempty(handles.IntegrationParams.MaskBitmap) || any(size(handles.IntegrationParams.MaskBitmap) ~= size(handles.State.Image)))
if (isempty(handles.IntegrationParams.MaskBitmap))
    handles.IntegrationParams.MaskBitmap = zeros(size(handles.State.Image));
end
...
if (0)
    ...
    finalMask = handles.IntegrationParams.MaskBitmap(...
        1:min(size(handles.State.Image, 1), size(handles.IntegrationParams.MaskBitmap, 1)), ...
        1:min(size(handles.State.Image, 2), size(handles.IntegrationParams.MaskBitmap, 2)) ...
        );
...
if (isempty(handles.State.MaskHandle) || ~ishandle(handles.State.MaskHandle) || shouldRedraw)
    if (~isempty(mask))
        %axes(handles.axes1);
        %hold on;
        ...
        if (gcf() == get(handles.PlotMenu, 'Parent'))
...
if (any(size(handles.State.Image) > size(handles.IntegrationParams.MaskBitmap)))
    handles.IntegrationParams.MaskBitmap = ExpandOrCropToMatch(handles.IntegrationParams.MaskBitmap, size(handles.State.Image), 1);
end
...
if ~isempty (filenameList)
    %     if (get(handles.plotTypeBox,'value') ~= 2)
    %         set(handles.plotTypeBox,'value',2);
    %     end
    ...
    if (isempty(handles.IntegrationParams.MaskBitmap))
        ...
        handles = guidata(hObject);
    elseif (any(size(handles.IntegrationParams.MaskBitmap) < size(handles.State.Image)))
        handles = ExpandMaskToMatchImage(hObject, handles);
    end
else
    handles.State.Image=[];
...
if (any(get(handles.plotTypeBox,'value') == [2 5 6]))
    plotit(hObject, [], 0, 0);
end
...
if handles.State.Image
    ...
    Status(strcat('SAXSi is showing: ', filenameList{vl}));
end
...
if (0) % Diamond patch - display sample name
    for i = 1:numel(listOfFiles)
        ...
        if (strcmp(ext, '.h5'))
            ...
            if (numel(parts) == 3 && strcmp(parts{1}, 'b21')) % Let's verify some more stuff
...
% TODO: Check this still works (some changes were made to "addingnew" function)
if (~isempty(newFileIndexes) && handles.DisplayOptions.AutoAddNewFiles)
    for i = 1:length(newFileIndexes)
        handles = AddCurveFile(hObject, handles, listOfFiles{i}, ...
            listOfFiles(newFileIndexes(i)), false, true);
    end
end
1;
...
UpdateIntegrationParamsDisplay(hObject);
plotit(hObject,handles,false,0);
SaveDefaultSettings(hObject)
    ...
    try
        for i = 1:numel(val)
            ...
            if (0) %strcmp(ext, '.h5')) % Read frames of h5 file (used in Diamond)
                ...
                for f = 1:numOfFrames
                    handles = AddCurveFile(hObject, handles, contents{val(i)}, [filePath ';' num2str(f)], false, false);
                end
            else
                handles = AddCurveFile(hObject, handles, contents{val(i)}, filePath, false, false);
            end
...
function [handles] = AddCurveFile(hObject, handles, displayName, filepath, shouldReintegrate, shouldPlot)

compare = @strcmp;
if (ispc) % Windows?... use only lower case to avoid false negative comparison
    compare = @strcmpi;
    %filepath = lower(filepath);
end

% get filename without folder
filepath = fullfile(filepath); % normalized path separators
[folder, filenameWithoutExt, ext] = fileparts(filepath);
filename = [filenameWithoutExt, ext];

alreadyInTheFilesList = false;

% the default curve position is N+1
curveIndex = numel(handles.State.Curves) + 1;

if (1) % Check if there is already a curve with this filename
    for i = 1:numel(handles.State.Curves)
        
        % Compare folder
        if (~compare(handles.State.Curves(i).Folder, folder))
            continue;
        end
        
        % Compare filename
        if (~isempty(handles.State.Curves(i).Filename) && compare(handles.State.Curves(i).Filename, filename))
            alreadyInTheFilesList = true;
        end
            
        if (~isempty(handles.State.Curves(i).ImageFilename) && compare(handles.State.Curves(i).ImageFilename, filename))
            alreadyInTheFilesList = true;
        end
        
        if (alreadyInTheFilesList)
            curveIndex = i;
            break;
        end
    end
...
%alreadyInTheFilesList = false; % for debug

% If not already on the list, or we are forcing integration
if (~alreadyInTheFilesList || shouldReintegrate)
    
    Status('Calculating...please wait');
    ...
    % Integrate (or read) the image
    [curve, xhiCurve, wasRead, filenameThatWasRead] = ...
        readOrIntigrate (filepath, ...
        GeneralOptions, CalibrationData, IntegrationParams, ...
        shouldReintegrate, handles.FastIntegrationCache);
    
    if (wasRead)
        % In case we have read a file with nm^-1 scale
        if (strcmp(curve.QScale, 'nm'))
            curve.QScale = 'A';
            curve.Q = 0.1 * curve.Q;
        end
    else % ~wasRead, handle saving the integrated file(s)
    ...
    if ((length(curve.Q)==length(curve.I)) & (length(curve.Q))) | ( ~isempty(xhiCurve.I))
        ...
        % TODO: Re-formulate the subtraction code
        ...
        if (wasRead && ~compare(filename, filenameThatWasRead))
            curve.ImageFilename = filename;
            curve.Filename = filenameThatWasRead;
        else
            curve.ImageFilename = [];
        ...
        if (~isempty(displayName))
            ...
            curve.DisplayName = curve.Filename;
        end
        ...
        % Add the curve
        AddCurve(hObject, handles, curve, xhiCurve, curveIndex, shouldPlot);
    end
else
    beep;
end

function AddCurve(hObject, handles, curve, xhiCurve, curveIndex, shouldPlot)

if (~exist('shouldPlot', 'var') || isempty(shouldPlot))
    shouldPlot = 1;
end

if (~exist('curveIndex', 'var') || isempty(curveIndex))
    % the default curve position is N+1
    curveIndex = numel(handles.State.Curves) + 1;
end

% This seemingly over-complicated code is necessary since the curves are
% structs
...
set (handles.DisplayedFilesList,'String', {handles.State.Curves.DisplayName});
...
switch (plotType)
    ...
    case 7 % intensity error
        % TODO
        [sOutFlag, handles] = plotitPixelPerBinCount(hObject, handles, sOutFlag);
        
    case 8 % rejected pixels (1d)
        % TODO
        [sOutFlag, handles] = plotitPixelPerBinCount(hObject, handles, sOutFlag);
        
    case 9 % rejected pixels (2d)
        % TODO
        [sOutFlag, handles] = plotitPixelPerBinCount(hObject, handles, sOutFlag);
...
function [h] = DrawQConic(hObject, handles, q, color, lineWidth)
...
if (0) % Plot the whole conic?
    ...
    for seg = 1:size(segments, 1)
        ...
        h(end+1) = plot(x, y, '-', 'LineWidth', lineWidth, 'Color', color);
    end
end
1;
...
function AutomaticallyAdjustIntensityRange(hObject, handles)
if (isempty(handles.State.DisplayedImage))
    return;
end

I = handles.State.DisplayedImage(:);
maximalIntensity = max(I);
minimalIntensity = min(I);

handles.DisplayOptions.IntensityRangeFor2d = [minimalIntensity maximalIntensity];
UpdateDisplayOptionsDisplay(hObject); % no need to save
...
if (preprocessingType > 0)
    ...
    if (0) % Test out some stuff regarding error estimation
        I = interp1(curve.Q, curve.I, handles.FastIntegrationCache.QImage(:));
        I = reshape(I, size(handles.State.Image));
        [curveTwiceIntegrated, ~] = Integrate (...
            I, maskIn, ...
            handles.CalibrationData, handles.IntegrationParams, ...
            handles.FastIntegrationCache);
        figure(11); loglog(curve.Q, curve.I); hold on; loglog(curveTwiceIntegrated.Q, curveTwiceIntegrated.I, '--k'); hold off
        figure(11); loglog(curve.Q, curve.IErr); hold on; loglog(curveTwiceIntegrated.Q, curveTwiceIntegrated.IErr, '--k'); hold off
        hold on; loglog(curve.Q, sqrt(curve.IErr.^2 - curveTwiceIntegrated.IErr.^2), '-.r'); hold off;
        hold on; loglog(curve.Q, (curve.IErr - curveTwiceIntegrated.IErr), ':r'); hold off;
    end
    ...
    switch (preprocessingType)
        ...
        case 3 % 1d-2d residuals
            factor = double(handles.State.Image) ./ I;
            factor = mean(factor(factor > 0));
            
            imageToDisplay = double(handles.State.Image) - I;
            %imageToDisplay = (double(handles.State.Image) - I) ./ double(handles.State.Image);
    ...
    if (0)
        ...
        K = 11;
        gauss2dKernel = gausswin(K) * gausswin(K)';
        gauss2dKernel = gauss2dKernel ./ sum(gauss2dKernel(:));
        imageToDisplay = conv2(imageToDisplay, gauss2dKernel, 'same');
    end
end
...
if (isAutoMaxIntensityEnabled)
    AutomaticallyAdjustIntensityRange(hObject, handles);
end
...
if (~shouldRedraw) % Just replace data, do not fully redraw
    if (~isempty(imageToDisplay))
        ...
        set(handles.axes1, 'clim', handles.DisplayOptions.IntensityRangeFor2d);
...
if ~isempty(imageToDisplay)
    ...
    if (gcf() == get(handles.PlotMenu, 'Parent')) % A fix for "export figure" which doesn't interfere with regular operation
...
if (handles.IntegrationParams.QMin > 0 && handles.IntegrationParams.QMax > handles.IntegrationParams.QMin)
    ...
    handles.State.QMinLineHandles = DrawQConic(hObject, handles, handles.IntegrationParams.QMin);
    
    % Draw the max conic
    handles.State.QMaxLineHandles = DrawQConic(hObject, handles, handles.IntegrationParams.QMax);
end
...
DisplayOptions = handles.DisplayOptions;
displayedLabels = get(handles.DisplayedFilesList, 'String');
shouldDisplayOnlySelected = get(handles.UseOnlySelectedCheckbox, 'Value');

if (~shouldDisplayOnlySelected)
    which = true(size(displayedLabels));
elseif (~isempty(displayedLabels))
    which = false(size(displayedLabels));
    tmp = get(handles.DisplayedFilesList, 'Value'); % gets a vector of values
    which(tmp) = true;
else
    which = [];
end
...
if (numOfFilesCurrentlyDisplayed)
    ...
    In.DisplayedLabels = displayedLabels(which);
    In.fid = handles.axes1;
    In.SP = DisplayOptions.SpreadCurvesMethod;
    In.flagedit = false;
    
    if (DisplayOptions.ShouldSubtractBackground)
        In.backgroundCurve = DisplayOptions.BackgroundCurve;
    else
        In.backgroundCurve = [];
    end
    
    if (get(handles.plotTypeBox,'Value')==1)
        
        curves = handles.State.Curves(which);
        
        if (handles.DisplayOptions.ShouldApply1dPreprocessing)
            for i = 1:numel(curves)
                
                %%
                [curve] = feval(handles.DisplayOptions.PreprocessingFunction, ...
                    curves(i), [], handles.DisplayOptions, handles.IntegrationParams);
                curves(i) = curve;
            end
        end
        
        handles.Out = readitAxes (curves, handles.DisplayOptions, handles.CalibrationData, In);
    elseif (get(handles.plotTypeBox,'Value')==3)
        try
...
try
    ...
    In.diff = 0;
    In.DisplayedLabels = filenamelist;
    In.fid = handles.axes1;
    In.SP = 1;
    In.flagedit = false;
    In.notfirst = false;
...
% --- Executes on button press in plotTypeBox.
function plotTypeBox_Callback(hObject, eventdata, handles)
% hObject    handle to plotTypeBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function plotTypeBox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to plotTypeBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
chosenMethod = get(handles.slist, 'Value');

if (chosenMethod == 6)
    answer = inputdlg('Enter region to scale by ("q [x]-[y]", "i [i1]-[i2]")', 'Region to scale by', 1, {''});
    
    if (isempty(answer))
        set(handles.slist, 'Value', handles.DisplayOptions.SpreadCurvesMethod);
        return;
    end
    
    %%
    matchResult = regexp(answer{1}, '(?<by>[qiQI])\s*(?<r1>\d*(?:\.\d*)?)\s*(?:-\s*)?(?<r2>\d*(?:\.\d*)?)', 'names');
    
    try
        matchResult.r1 = str2double(matchResult.r1)
        matchResult.r2 = str2double(matchResult.r2)
    catch
        errordlg('Something wrong with the given numbers. Could not ');
        return;
    end

    %%
    switch (lower(matchResult.by))
        case 'q'
            handles.DisplayOptions.CurveScalingRegion = sort([matchResult.r1 matchResult.r2]);
            
        case 'i'
            curves = handles.State.Curves;
            handles.DisplayOptions.CurveScalingRegion = sort(curves(1).Q([matchResult.r1 matchResult.r2]));
            
        otherwise
            errordlg('Unknown prefix, please use "q" or "i"');
            return;
    end
...
handles.DisplayOptions.SpreadCurvesMethod = chosenMethod;
SaveAndUpdateDisplayOptions(hObject);
plotit(hObject,handles);
...
for i = 1:numel(handles.State.QMinLineHandles)
    set(handles.State.QMinLineHandles(i), 'XData', [], 'YData', []);
end
...
for i = 1:numel(handles.State.QMaxLineHandles)
    set(handles.State.QMaxLineHandles(i), 'XData', [], 'YData', []);
end
...
try
    for i = 1:length (curves)
        try
            ...
            handles = AddCurveFile(hObject, handles, [], fullfile(folders{i}, filenames{i}), true, false);
        catch ex
...
if fn
    try
        ...
        handles.IntegrationParams.MaskBitmap = MaskBitmap;
        handles.FastIntegrationCache.Clear();
        SaveDefaultIntegrationSettings(hObject);
    catch err
        msgbox(err.message);
    end
end
...
% handles    structure with handles and user data (see GUIDATA)
%handles.IntegrationParams.MaskBitmap = zeros(size(handles.State.Image));
handles.IntegrationParams.MaskBitmap = [];
% Update handles structure
guidata(hObject, handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%handles.IntegrationParams.MaskBitmap=MakemaskMat(handles.maskIn);
...
if fn
    ...
    MaskBitmap = handles.IntegrationParams.MaskBitmap;
...
function SaveDisplaySettings(hObject, handles)
if (~exist('handles', 'var') || isempty(handles))
    handles = guidata(hObject);
end

% Avoid saving the currently loaded setting
if (handles.State.IsWithinLoadSettings)
    return;
end

computerName = getenv('COMPUTERNAME');

if (ispc() && ~isempty(computerName))
    filename = ['settings.display.' computerName '.mat'];
else
    filename = 'settings.display.mat';
end
...
try
    ...
    save (fullfile(folder, filename), '-mat', 'DisplayOptions', 'GeneralOptions');
catch
    error ('Error in saving display settings');
    beep
    msgbox('Error in saving display settings', 'Error', 'error');
end


function [h] = LoadDisplaySettings(hObject, handles)
if (~exist('handles', 'var') || isempty(handles))
    handles = guidata(hObject);
end
...
try
    ...
    if (~exist(filename, 'file'))
        filename = 'defaults.sxs'; % revert to old settings filename, just in case
    end
    ...
    % Override "AutoAddNewFiles" for now. Not sure users would want this option to persist
    handles.DisplayOptions.AutoAddNewFiles = 0;
    % Override "ShouldSearchFilesRecursively" for now. Not sure users would want this option to persist
...
function SaveDefaultIntegrationSettings(hObject, handles)
if (~exist('handles', 'var') || isempty(handles))
    handles = guidata(hObject);
end
...
filename = 'settings.integration.mat';
[folder, ~, ~] = fileparts(mfilename('fullpath'));
SaveIntegrationSettings(hObject, fullfile(folder, filename));


function SaveIntegrationSettings(hObject, filepath)
handles = guidata(hObject);
...
try
    ...
    save (filepath, '-mat', 'CalibrationData', 'IntegrationParams');
catch
    error ('Error in saving integration settings');
    beep
    msgbox('Error in saving integration settings', 'Error', 'error');
end


function [h] = LoadDefaultIntegrationSettings(hObject, handles)
if (~exist('handles', 'var') || isempty(handles))
    handles = guidata(hObject);
end
...
h = LoadIntegrationSettings(hObject, fullfile(folder, filename));


function [h] = LoadIntegrationSettings(hObject, filepath)
handles = guidata(hObject);
handles.State.IsWithinLoadSettings = 1;
...
try
    ...
    if (~isfield(h, 'CalibrationData')) % Old file?...
        LoadOldIntegrationSettingsFile(hObject, filepath)
    end
    ...
    if (isfield(h, 'MaskBitmap')); handles.IntegrationParams.MaskBitmap = h.MaskBitmap; end % relevant for older settings files
...
function LoadOldIntegrationSettingsFile(hObject, filepath)
handles = guidata(hObject);

vars = load (filepath, '-mat', 'handles', 'alpha', 'beta', 'bmx', 'bmy', 's2d', ...
...
handles = guidata(hObject);
SaveDisplaySettings(hObject, handles);
SaveDefaultIntegrationSettings(hObject, handles);

function LoadDefaultSettings(hObject)
handles = guidata(hObject);
LoadDisplaySettings(hObject, handles);
LoadDefaultIntegrationSettings(hObject, handles);
...
[filename, folder] = uiputfile('*.sxs', 'Save calibration/integration settings file', currentFolder);

if filename
    len = length(filename);
    
    if ~(strcmp(lower(filename(len-3:len)),'.sxs'))
        filename=[filename '.sxs'];
    end
    
    try
        SaveIntegrationSettings(hObject, [folder filename]);
    catch
        msgbox('Error', 'Error in saving', 'error');
        beep
    end
end
...
function loadsettingbu_Callback(hObject, eventdata, handles)
% hObject    handle to loadsettingbu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
currentFolder = handles.DisplayOptions.GetDirectory();

[filename, folder] = uigetfile('*.sxs', 'Load integration settings file', currentFolder);
if (filename == 0)
    return;
end

filename = fullfile(folder, filename);
loadedSettings = LoadIntegrationSettings(hObject, filename);

% This wasn't the default's, save as defaults
SaveDefaultIntegrationSettings(hObject, handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if (all(get(handles.plotTypeBox,'Value') ~= [2 5 6]))
...
handles.IntegrationParams.DezingerPercentile = str2double(get(hObject, 'String'));
set(hObject, 'String', sprintf('%g', handles.IntegrationParams.DezingerPercentile));
SaveDefaultIntegrationSettings(hObject);
1;
...
try
    for i = 1:length (outfilename)
        ...
        handles = AddCurveFile(hObject, handles, [], fileFullPaths{i}, true, false);
    end
catch err
    1;
end
...
if len>1
    ...
    try
        ...
        handles = AddCurveFile (hObject, handles, [], outputPath, 0, 1);
        
        Status(sprintf('Successfully created: %s', outputFilename));
    catch err
        disp ('There is a problem averaging/summing');
        Status('There is a problem averaging/summing');
        disp (err.message);
    end
end
...
if (abs(min(rec))>0)
    ...
    newmsk = false(size(handles.IntegrationParams.MaskBitmap));
    for i=1:len
        waitbar(i/len)
...
set(handles.plotTypeBox, 'Value', DisplayOptions.CurrentPlotType);
...
set(handles.cLimMinnew, 'String', sprintf('%g', DisplayOptions.IntensityRangeFor2d(1)));
set(handles.cLimMaxnew, 'String', sprintf('%g', DisplayOptions.IntensityRangeFor2d(2)));
set(gca, 'clim', handles.DisplayOptions.IntensityRangeFor2d);

if (DisplayOptions.AutoMaxIntensity)
    set(handles.cLimMinnew, 'BackgroundColor', [0.7 1 0.7]);
    set(handles.cLimMaxnew, 'BackgroundColor', [0.7 1 0.7]);
else
    set(handles.cLimMinnew, 'BackgroundColor', [0.88 0.88 0.88]);
    set(handles.cLimMaxnew, 'BackgroundColor', [0.88 0.88 0.88]);
end
...
set(handles.subtractCheckbox, 'Value', DisplayOptions.ShouldSubtractBackground);
...
set(handles.dezingerTypePopupMenu, 'Value', IntegrationParams.DezingerType + 1);
set(handles.dezingerPer, 'Enable', onoff{1 + (IntegrationParams.DezingerType > 0)});
...
% Adjust image color limits
%set(gca,'clim',[str2num(get(handles.cLimMinnew,'string')), str2num(get(handles.cLimMaxnew,'string'))]);

% reset automatic color scaling
handles.DisplayOptions.AutoMaxIntensity = 0;

% "standardize" number display
value = str2double(get(hObject,'String'));
set(hObject, 'String', sprintf('%g', value));

handles.DisplayOptions.IntensityRangeFor2d(1) = value;
...
% Adjust image color limits
%set(gca,'clim',[str2double(get(handles.cLimMinnew,'string')), str2double(get(handles.cLimMaxnew,'string'))]);
...
handles.DisplayOptions.IntensityRangeFor2d(2) = value;
...
Status(sprintf('Draw mask %s - double click to finish', roiShape));
% Update handles structure
guidata(hObject, handles);
...
% Update handles structure
handles = ExpandMaskToMatchImage(hObject, handles);
handles.IntegrationParams.MaskBitmap(handles.State.Image < 0) = 1;
...
function HandlePreZoom(hObject, eventdata)
1;
...
if (strcmp(zoomHandle.Enable, 'off'))
    ...
    set(zoomHandle, 'ActionPreCallback', @HandlePreZoom);
...
while but == 1
    ...
    elseif (any(handles.DisplayOptions.CurrentPlotType == [4 5]))
        q = handles.CalibrationData.GetQForXY(xi, yi);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
sum2dimages(handles.DisplayOptions.GetDirectory(), 0);

% --------------------------------------------------------------------
function avg2DMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to avg2DMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
sum2dimages(handles.DisplayOptions.GetDirectory(), 1);

% --------------------------------------------------------------------
function subtract2DMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to subtract2DMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
subtract2dimages(handles.DisplayOptions.GetDirectory());
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.plotTypeBox, 'Value', 1);
plotTypeBox_Callback(hObject, eventdata, handles)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.plotTypeBox, 'Value', 3);
plotTypeBox_Callback(hObject, eventdata, handles)
...
if (handles.DisplayOptions.AutoMaxIntensity)
    AutomaticallyAdjustIntensityRange(hObject, handles);
end
...
if (~exist('nameOfStep', 'var') || isempty(nameOfSteps))
    lp.TextFormat = '{StepsDone:%d}/{TotalSteps:%d} steps done ({PercentageDone:%0.1f%%}). Approximately {EstimatedTimeLeft:%0.0f} more seconds.';
else
    lp.TextFormat = ['{StepsDone:%d}/{TotalSteps:%d} ' nameOfSteps ' done ({PercentageDone:%0.1f%%}). Approximately {EstimatedTimeLeft:%0.0f} more seconds.'];
end
...
lp.PercentageDone = (stepsDone - 1) / lp.TotalSteps * 100;
...
try
    for i = 1:length (outfilename)
        if (~LongProcessStep(lp, i, [])); break; end
        handles = AddCurveFile(hObject, handles, outfilename{i}, fileFullPaths{i}, false, false);
    end
catch err
    1;
end
...
% TODO: Put spreading code in another function (to be shared with the
% plotting function)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[fn, folder] = uiputfile('*.svg', 'Save image file (SVG)');
...
if fn
    ...
    %addpath('djoshea-matlab-save-figure-8966f29');
    %saveFigure(filepath, gcf());
...
Status(sprintf('Lamellar q = %0.5g, d = 2pi/x = %0.5g', q, 2*pi/q));
...
Status(sprintf('Hexagonal q = %0.5g, a = (4pi/sqrt(3))/x = %0.5g', q, (4*pi/sqrt(3))/q));
...
Status(sprintf('Cubic q = %0.5g, d = 2pi/x = %0.5g', q, 2*pi/q));
...
% --------------------------------------------------------------------
function Save2dImageWithoutOverlayMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to Save2dImageWithoutOverlayMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[fn, folder] = uiputfile('*.tif', 'Save image file (TIF)');

if fn
    filepath = [folder fn];
    cl = get(handles.axes1,'clim');
    
    rgb = Matrix2Image(handles.State.DisplayedImage, cl(1), cl(2), colormap(handles.axes1));
    mask = GetMask(hObject, handles);
    mask = logical(bsxfun(@times, mask, ones(1, 1, 3)));
    rgb(mask) = 0;
...
% --------------------------------------------------------------------
function Tools1DMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to Tools1DMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
onoff = {'off', 'on'};
set(handles.applyPreprocessingMenuItem, 'Checked', onoff{1 + (handles.DisplayOptions.ShouldApply1dPreprocessing)});
1;


% --------------------------------------------------------------------
function Tools2DMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to Tools2DMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% --- Executes on selection change in dezingerTypePopupMenu.
function dezingerTypePopupMenu_Callback(hObject, eventdata, handles)
% hObject    handle to dezingerTypePopupMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns dezingerTypePopupMenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from dezingerTypePopupMenu
handles.IntegrationParams.DezingerType = get(hObject, 'Value') - 1;
UpdateIntegrationParamsDisplay(hObject);
SaveDefaultIntegrationSettings(hObject);


% --- Executes during object creation, after setting all properties.
function dezingerTypePopupMenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to dezingerTypePopupMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- If Enable == 'on', executes on mouse press in 5 pixel border.
% --- Otherwise, executes on mouse press in 5 pixel border or over dezingerTypePopupMenu.
function dezingerTypePopupMenu_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to dezingerTypePopupMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
1;

% --- Executes on key press with focus on dezingerTypePopupMenu and none of its controls.
function dezingerTypePopupMenu_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to dezingerTypePopupMenu (see GCBO)
...
switch(eventdata.Key)
    case 'f12'
        selectedIndex = get(handles.DisplayedFilesList, 'Value');
        if (isempty(selectedIndex) || selectedIndex > numel(handles.State.Curves))
            return;
        end
        
        curve = handles.State.Curves(selectedIndex);
        if (isempty(curve.Filename)); return; end;
        if (IsExtensionOfImage(curve.Filename))
            imageFilepath = fullfile(curve.Folder, curve.Filename);
        end
        
        options = GeneralOptionsClass();
        options.CopyFrom(handles.GeneralOptions);
        options.ShouldLoadIntegratedFileInsteadOfIntegrating = 0;
        
        integrationParams = IntegrationParamsClass();
        integrationParams.CopyFrom(handles.IntegrationParams);
        ...
        %%
        % Integrate (or read) the image
        [curve] = readOrIntigrate (filepath, ...
            options, handles.CalibrationData, integrationParams, ...
            0, handles.FastIntegrationCache);
...
% --- Executes on button press in subtractCheckbox.
function subtractCheckbox_Callback(hObject, eventdata, handles)
% hObject    handle to subtractCheckbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.DisplayOptions.ShouldSubtractBackground = ...
    ~handles.DisplayOptions.ShouldSubtractBackground;

if (handles.DisplayOptions.ShouldSubtractBackground)
    set(handles.subtractCheckbox, 'Value', 1);
else
    set(handles.subtractCheckbox, 'Value', 0);
end
...
% hObject    handle to useAsBackgroundButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
selectedIndex = get(handles.DisplayedFilesList,'Value');
curve = handles.State.Curves(selectedIndex);
handles.DisplayOptions.BackgroundCurve = curve;

handles.DisplayOptions.ShouldSubtractBackground = false;
subtractCheckbox_Callback(hObject, [], handles);

1;

% --------------------------------------------------------------------
function applyPreprocessingMenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to applyPreprocessingMenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if (handles.DisplayOptions.ShouldApply1dPreprocessing)
    handles.DisplayOptions.ShouldApply1dPreprocessing = false;
    return;
end

if (~isempty(handles.DisplayOptions.PreprocessingFunction))
    [fileName, pathName] = uigetfile('*.m', 'Select the MATLAB code file', fullfile(cd(), [handles.DisplayOptions.PreprocessingFunction '.m']));
else
...
handles.DisplayOptions.ShouldApply1dPreprocessing = true;
cd(pathName);
[~, fileNameNoExt] = fileparts(fileName);
handles.DisplayOptions.PreprocessingFunction = fileNameNoExt;
set(handles.applyPreprocessingMenuItem, 'Checked', 'on');
