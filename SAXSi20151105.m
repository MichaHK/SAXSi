...
% Last Modified by GUIDE v2.5 28-Jul-2015 19:04:14
...
%     end
    ...
    if (numel(size(image))> 2) % Image has too many dimensions
...
elseif (numel(val) > 0)
    ...
    try
        for i = 1:numel(val)
            ...
            handles = addingnew(hObject, handles, fullPathsList{val(i)}, false, false);
        end
    catch err
        beep;
    end
...
for i = 1:numel(handles.State.Curves)
    if (strcmp(handles.State.Curves(i).Folder, folder) && ...
...
if ~alreadyInTheFilesList || shouldReintegrate % If not already on the list 
    ...
    % Integrate the image
    [curve, xhiCurve, wasRead] = ...
        readOrIntigrate (filepath, ...
        CalibrationData, IntegrationParams, maskIn, ...
...
function [sOutFlag, handles] = plotit2D (hObject, handles, shouldRedraw, sOutFlag)
...
shouldDisplayOnlySelected = get(handles.DisplayOnlySelectedCheckbox, 'Value');
...
outfilename = get(handles.DisplayedFilesList, 'String');
set(handles.DisplayedFilesList,'String',[]);

filenames = {handles.State.Curves.Filename};
folders = {handles.State.Curves.Folder};
...
while but == 1
    ...
    str = sprintf('x=%i y=%i q=%f', xi, yi, q);
...
selectedFileIndexes = [];
if (get(handles.DisplayOnlySelectedCheckbox, 'Value'))
...
% --- Executes on button press in DisplayOnlySelectedCheckbox.
function DisplayOnlySelectedCheckbox_Callback(hObject, eventdata, handles)
% hObject    handle to DisplayOnlySelectedCheckbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plotit (hObject, handles);
...
for i = 2:numel(X)
    if (any(size(X{1}) ~= size(X{i})) || any(abs(X{1} - X{i}) > 1e-9))
