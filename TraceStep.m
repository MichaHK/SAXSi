function [success, rn,tt,X] = TraceStep(r,tt,data,stepsize,scansize,scannum,maxiter,sigmasInsideThreshold)
...
for a=1:maxiter2
    ...
    % now we do the interpolation onto these point producing
    % a vector of z values
    %     disp ('min of scan')
    %     disp (min(rscanvec));
    %    z = interpolate_im3(data,rscanvec); %changed by R. Beck
    %z = interpolate_im3(data,rscanvec/pixelsize); % Commented-out by Ram Avinery
    ...
    yy=rscanvec(:,2);
    xx=rscanvec(:,1);
    z = interp2(1:size(data, 1), 1:size(data, 2), data', yy, xx, 'linear');
