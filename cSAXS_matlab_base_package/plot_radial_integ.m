...
%
% $Revision: 1.14 $  $Date: 2011/11/25 15:11:29 $
% $Author: bunk $
% $Tag: $
%
% Description:
...
%
% February 19th 2009, Oliver Bunk:
% average only positive intensities (i.e., valid pixels)
% 
% September 4th 2009, Oliver Bunk: add Axis parameter
%
% June 9th 2008, Oliver Bunk: 1st documented version
%
function [x_values_plotted,y_values_returned] = plot_radial_integ(filename_masks, varargin)
...
if (nargin < 1)
    ...
    fprintf('''PixelSize_mm'',<value in mm>         pixel size for q-calculation, default is %.3f mm\n',pixel_size_mm);
    fprintf('''DetDist_mm'',<value in mm>           sample to detector distance for q-calculation, default is %.3f mm\n',det_dist_mm);
    fprintf('''E_keV'',<value in keV>               x-ray energy for q-calculation, default is %.3f mm\n',E_keV);
...
%             end
        ...
        I_times = ones(length(radius),1,1);
...
I_all = zeros(no_of_radii,no_of_segments,1) -1;
for (ind1=1:no_of_radii)
    for (ind2=1:no_of_segments)
        no_of_el = 0;
